...
1. **每次 `call()` 结束** —— 默认行为（`FlushTrigger.always()`），可改 `NEVER` / `THROTTLED(Duration)`。**落到实现是 `MemoryFlushMiddleware#onAgent` 里的 `next.apply(input).concatWith(Mono.defer(() -> doFlush(agent, rc)).subscribeOn(Schedulers.boundedElastic())…)`**——注意它挂在 **`concatWith`** 上，所以 flush 是这条流的一部分、**会推迟 `onComplete`**；真正把执行挪出调用线程的是 `subscribeOn(boundedElastic())`。（`MemoryFlushMiddleware` 的**类注释写的是 `doOnComplete`，和实现不一致**，以代码为准。详见 **6.0 更正 ①** 与 **6.2 锚点 1**）
...
.compaction(CompactionConfig.builder()
    .triggerMessages(80)
    .truncateArgs(CompactionConfig.TruncateArgsConfig.builder()   // ← 这一段是「显式开启」
        .maxArgLength(2000)
